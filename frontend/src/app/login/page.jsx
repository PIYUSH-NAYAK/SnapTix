...
const floatingCards = [
  { label: "🎵 Coldplay Tour", sub: "Mumbai · Jun 2026", color: "from-pink-500/30 to-purple-600/30", x: "10%", y: "12%", rotate: -6 },
  { label: "🏏 India vs AUS", sub: "Chennai · Sep 2026", color: "from-blue-500/25 to-indigo-500/25", x: "60%", y: "8%", rotate: 5 },
  { label: "🎭 Sunburn Festival", sub: "Goa · Dec 2026", color: "from-orange-500/25 to-pink-500/25", x: "5%", y: "62%", rotate: 4 },
  { label: "🎤 AR Rahman Live", sub: "Delhi · Aug 2026", color: "from-green-500/20 to-teal-500/20", x: "58%", y: "68%", rotate: -4 },
];
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    toast.promise(p, { loading: "Logging in...", success: "Welcome back!", error: (err) => err.message });
  };
  ...
  const handleGoogle = async () => {
    ...
    const p = signInWithGoogle()
      ...
      .catch((err) => { throw new Error(err.message); })
      .finally(() => setLoading(false));
    toast.promise(p, { loading: "Connecting...", success: "Welcome back!", error: (err) => err.message });
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white flex pt-[calc(3rem+1px)]">
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden items-center justify-center">
        {/* Gradient orbs */}
        <div className="absolute top-1/4 left-1/4 w-72 h-72 rounded-full bg-pink-500/20 blur-3xl animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full bg-purple-600/15 blur-3xl animate-pulse" style={{ animationDelay: "1s" }} />
        ...
        >
          ...
          <p className="text-gray-400 text-lg max-w-xs mx-auto leading-relaxed">
            NFT-secured tickets for the best events across India
          </p>
          ...
          <div className="mt-8 grid grid-cols-2 gap-3 max-w-xs mx-auto">
            {[
              ...
              { icon: "🎭", label: "Theatre & Arts" },
              { icon: "🔐", label: "NFT Secured" },
            ].map((f) => (
              <div key={f.label} className="flex items-center gap-2 bg-white/5 rounded-xl px-3 py-2 border border-white/10">
                <span>{f.icon}</span>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        ...
        >
          ...
          <p className="text-gray-400 mb-8 text-sm">Sign in to access your tickets and events</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                required
                className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:outline-none focus:border-pink-500 focus:bg-white/8 transition text-white placeholder-gray-600"
              />
            </div>
            ...
            <div>
              <div className="flex justify-between mb-1.5">
                ...
                <Link href="/forgot-password" className="text-xs text-pink-400 hover:text-pink-300 transition">
                  Forgot password?
                </Link>
              </div>
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            ...
            <Link href="/signup" className="text-pink-400 hover:text-pink-300 font-medium transition">
              Create one free
            </Link>
          </p>
        </motion.div>
